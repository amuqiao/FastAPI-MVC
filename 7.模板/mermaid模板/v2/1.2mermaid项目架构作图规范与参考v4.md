...
四种图表**本质上不同**，各有明确的使用场景，不能互换。

| 维度 | 架构设计图（系统集成） | 端到端流程图（业务流程） | 数据模型关系图（表关系） | 类层级关系图（代码对象设计） |
|------|----------------------|----------------------|----------------------|--------------------------|
| **回答的问题** | 系统由哪些组件构成？它们如何部署和连接？ | 一个请求/数据如何在系统中流转？ | 数据如何组织和持久化？表与表之间如何关联？ | 这个模块的代码如何组织？抽象层次、继承树和组合关系如何体现设计意图？ |
| **视角** | 静态结构视图 | 动态流程视图 | 静态数据视图 | 静态代码结构视图 |
| **流向** | `TB`（从上到下，强调分层） | `LR`（从左到右，强调时序/步骤） | `TB`（按业务领域分组，强调数据归属） | `TB`（从抽象到具体，强调继承层次） |
| **关注点** | 组件、层级、依赖关系 | 触发源、处理步骤、数据转换、输出 | 表结构、外键引用、逻辑关联边界 | 抽象类、接口、继承树、组合关系、设计模式 |
| **节点代表** | 服务 / 组件 | 处理步骤 | **数据库表**（含核心字段） | **类 / 接口 / 抽象基类**（含 stereotype 与关键方法） |
| **箭头代表** | 调用 / 部署关系 | 数据流转顺序 | **外键引用（实线）/ 逻辑关联（虚线）** | **继承（实线）/ 接口实现（虚线）/ 组合（带语义标签）** |
| **类比** | 建筑平面图（房间布局） | 消防演练路线图（人怎么走） | 户型图（房间内部陈设与连通） | 建筑构件施工图（材料规格与受力结构） |
...
架构图（全局结构）
    ...
    └── 类层级关系图（某个模块/层次的代码对象设计）
    └── ...
```

### 组合使用

做完整项目分析时，四图配合使用，逐层深入：
...
第三步：数据模型图  →  理解数据存储边界（表结构、外键约束、版本快照设计）
    ↓
第四步：类层级关系图  →  理解代码对象设计（抽象层次、继承树、设计模式意图）
```

**一个系统对应一张架构图，但可以有多张业务流程图、多张数据模型关系图和多张类层级关系图**（按业务域/模块拆分，各自独立梳理）。
