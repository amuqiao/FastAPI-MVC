...
| **一图一职责** | 数据模型关系图回答“数据怎么存、怎么关联”；类层级关系图回答“代码怎么组织、抽象层次是什么”；不要在一张图里同时硬塞两类问题 |
| **先选语义图型** | 先判断当前更适合数据模型关系图还是类层级关系图，再复用本 reference 的风格 |
| **多图时先列清单** | 如果需要多张图，先列出每张图各自回答的问题，再依次输出 |
| **避免图文重复** | 图负责表达结构骨架，正文只补关键设计判断，不重复逐节点转述 |
