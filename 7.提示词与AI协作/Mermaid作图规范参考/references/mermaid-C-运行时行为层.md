...
| 设计原则 | 说明 |
|----------|------|
| **先判定是“状态”还是“消息”** | 如果问题核心是“某个实体处于什么状态、怎么切换”，画状态机图；如果核心是“多个参与者如何交互、谁在等待谁”，画时序图 |
| **一张图只用一种 Mermaid 语法** | `stateDiagram-v2` 和 `sequenceDiagram` 不能混写；不要把状态节点、参与者消息和 flowchart 语法塞进同一张图 |
| **不要继承 A/B 的 flowchart 习惯** | 本文件图型不支持 `classDef`、`linkStyle`、`subgraph`；如果你下意识想分层上色，说明当前可能选错图型了 |
| **状态名贴源码，事件名写语义** | 状态机图中的状态名优先复用枚举值/字段值原文；箭头标签再用中文解释触发事件，避免图上出现“状态翻译”和源码脱节 |
| **时序图优先保留等待关系** | 宁可省略次要字段，也要保留谁发起、谁阻塞、谁异步回推这三类关键信息；否则图会退化成普通流程图 |
| **长链路先拆阶段再出图** | 一条链路同时覆盖“请求接入 + 异步执行 + 回调落库 + 推送前端”时，先判断是否应拆成两张图，而不是强行压成一张超长时序图 |
...
| **一图一职责** | 状态机图回答“状态如何变化”；时序图回答“消息如何流转、谁在等谁”；不要把状态切换和跨组件消息硬塞进同一张图 |
| **先选语义图型** | 先判断当前更适合状态机图还是时序图，再复用本 reference 的风格 |
| **多图时先分阶段** | 如果链路太长需要多张图，先说明每张图各自覆盖的阶段或问题，再依次输出 |
| **避免装饰性补图** | 如果文字已经足够解释当前运行时行为，就不要为了完整感额外补图 |
