...
- 源序列输入：`[32, 10]` （32个样本，每个样本10个词）
- 目标序列输入：`[32, 9]` （32个样本，每个样本9个词）
- 嵌入层输出：`[32, 10, 512]` 和 `[32, 9, 512]` （512维模型维度）
- 编码器输出：`[32, 10, 512]` （保持512维模型维度）
- 解码器输出：`[32, 9, 512]` （保持512维模型维度）
- 线性层输出：`[32, 9, 10000]` （10000维词表空间）
- Softmax输出：`[32, 9, 10000]` （概率分布）
- 预测结果：`[32, 9]` （每个位置预测一个词索引）

这样，模型就能完成从源序列到目标序列的转换，其中d_model是模型内部的核心维度，vocab_size是连接模型内部表示和最终词预测的关键维度。
