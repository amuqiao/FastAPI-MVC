...
> **文档职责**：只解决 macOS 上的 `Homebrew`、`Node.js`、`npm` 环境准备问题，作为 Codex CLI 的前置依赖文档。  
> **适用场景**：第一次在 Mac 上搭建 Node 工具链，或者本机 `brew` / `node` / `npm` 尚不可用。  
> **不包含内容**：不讲 Codex 安装、不讲提示词、不讲代码开发流程。  
> **阅读顺序**：先读本文，再读 `2.Codex_CLI_安装与配置指南.md`。  
